...
/// D41 slow-tracking reserve check price.
/// Downward instant. Upward bounded by `reserve_check_price_max_increase_per_hour_bps` per hour.
/// Uses its own dedicated timestamp `reserve_check_price_last_update_at` (NOT `last_price_update_at`)
/// to avoid the rubber-band attack where many dust-filtered small txs keep `last_price_update_at`
/// stale, then a single tx would ramp the price by an inflated `elapsed`.
pub fn update_reserve_check_price(
    config: &mut ConfigAccount,
    current_oracle: u128,
    now: i64,
) -> Result<()> {
    if config.reserve_check_price_scaled == 0 {
        config.reserve_check_price_scaled = current_oracle;
        config.reserve_check_price_last_update_at = now;
        return Ok(());
    }
    if current_oracle <= config.reserve_check_price_scaled {
        config.reserve_check_price_scaled = current_oracle;
        config.reserve_check_price_last_update_at = now;
        return Ok(());
    }
    // Compute max upward delta over elapsed time using the dedicated reserve-check timestamp.
    let elapsed = now
        .saturating_sub(config.reserve_check_price_last_update_at)
        .max(0) as u128;
    let bps = config.reserve_check_price_max_increase_per_hour_bps as u128;
    let prev = config.reserve_check_price_scaled;
    // increase = prev * bps * elapsed / (3600 * 10_000); use checked math throughout.
    let max_increase: u128 = prev
        .checked_mul(bps)
        .ok_or(DominionError::ArithmeticOverflow)?
        .checked_mul(elapsed)
        .ok_or(DominionError::ArithmeticOverflow)?
        .checked_div(3600 * 10_000)
        .ok_or(DominionError::ArithmeticOverflow)?;
    let ramped = prev.saturating_add(max_increase);
    config.reserve_check_price_scaled = current_oracle.min(ramped);
    config.reserve_check_price_last_update_at = now;
    Ok(())
}
