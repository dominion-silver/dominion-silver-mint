...
// BOUNDS that the timelocked path enforces (PREMIUM_BPS_HARD_CEILING,
// PREMIUM_BPS_COMBINED_FLOOR, sane staleness floor/ceiling). This way a
...
pub fn dev_set_max_staleness_handler(
...
///   - each <= PREMIUM_BPS_HARD_CEILING (3000 bps = 30%)
...
pub fn dev_set_premiums_handler(
    ...
    require!(
        mint_bps <= PREMIUM_BPS_HARD_CEILING,
        DominionError::PremiumTooHigh
    );
    require!(
        redeem_bps <= PREMIUM_BPS_HARD_CEILING,
        DominionError::PremiumTooHigh
    );
    require!(
