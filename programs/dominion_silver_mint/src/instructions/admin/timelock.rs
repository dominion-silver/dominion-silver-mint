...
pub fn cancel_handler(ctx: Context<CancelTimelocked>, nonce: u64) -> Result<()> {
    ...
    let action = match tl.action_disc {
        ...
        x if x == TimelockAction::SetTreasuryMinReserve as u8 => {
            Some(TimelockAction::SetTreasuryMinReserve)
        }
    ...
    if let Some(a) = action {
        match a {
            ...
            TimelockAction::SetTreasuryMinReserve => config.pending_min_reserve_nonce = None,
