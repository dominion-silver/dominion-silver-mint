...
// represents.
// the redirect-then-premint pair is CLOSED, and closed by REMOVAL. The destination is an
...
// WHAT THIS DOES NOT PROTECT, and in config/mainnet-authorities.json is the answer to it: tokens
...
// `SetInventoryWallet` and its handler are DELETED, not restricted.
// The round-7 version kept the first binding instant on the argument that with the field unset there
// was nothing to redirect. refuted it: compromise the Ops key DURING the ceremony, before the
