...
/// `unpause` demands an active guardian distinct from the admin, so the seven copies of
...
/// Admin-signed cancel, so a test can reach the "nothing armed" state `unpause` now
