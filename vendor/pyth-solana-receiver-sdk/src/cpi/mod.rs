...
pub fn post_update<'info>(
    ...
    let ix = {
        // DOMINION VENDOR PATCH (2026-05-19): upstream's
        // `AnchorSerialize::try_to_vec(&params)` is the bare-trait UFCS
        // form modern rustc rejects as E0782 (pyth never fixed it
        // through HEAD). `params.try_to_vec()` is the SAME serialization
        // via the unambiguous method-call form: byte-for-byte identical
        // output, only the call syntax changed.
        let mut ix_data = params
            .try_to_vec()
